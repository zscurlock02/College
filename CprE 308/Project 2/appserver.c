...
void* requestHandler() {
	...
	while(run || commandBuffer -> size > 0) {
		...
		if(strcmp(commandTokens[0], "CHECK") == 0 && numTokens == 2) {
			...
			for(i = 0; i < numTransactions; i++) {
				for(j = i; j < numTransactions; j++) {
					if(transactionAccounts[j] < transactionAccounts[i]) {
						temp = transactionAccounts[i];
						transactionAccounts[i] = transactionAccounts[j];
						transactionAccounts[j] = temp;
						temp = transactionAmounts[i];
						transactionAmounts[i] = transactionAmounts[j];
						transactionAmounts[j] = temp;
					}
				}
			}

			for(i = 0; i < numTransactions; i++){
				pthread_mutex_lock(&(accounts[transactionAccounts[i]-1].lock));
            }
			...
			for(i = numTransactions - 1; i >=0; i--){
				pthread_mutex_unlock(&(accounts[transactionAccounts[i]-1].lock));
            }
		}
		else {
