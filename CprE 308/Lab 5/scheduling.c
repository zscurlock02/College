...
int first_come_first_served(process proc[], int t)
{
  ...
  for(i = 0; i < NUM_PROCESSES; i++){
    if(proc[i].arrivaltime <= t && !proc[i].finished){
      if((first < 0) || (proc[i].arrivaltime < proc[first].arrivaltime)){
        first = i;
      }
    }
  }
...
int shortest_remaining_time(process proc[], int t)
{
  ...
  for(i = 0; i < NUM_PROCESSES; i++){
    if(proc[i].arrivaltime <= t && !proc[i].finished){
      if((shortTime < 0) || (proc[i].remainingtime < proc[shortTime].remainingtime)){
        shortTime = i;
      }
    }
  }
...
int round_robin(process proc[], int t)
{
  ...
  for(i = 0; i < NUM_PROCESSES; i++){
    if(!proc[i].finished && proc[i].arrivaltime <= t){
      ...
      if(roundRobin < 0 || time[i] < time[roundRobin]){
        roundRobin = i;
      }
    }
  }
  time[roundRobin] = t + 1.1;
  return roundRobin;
}
...
int round_robin_priority(process proc[], int t)
{
  ...
  for(i = 0; i < NUM_PROCESSES; i++){   
    if(!proc[i].finished && proc[i].arrivaltime <= t){
      ...
      if (roundRobin < 0 || proc[i].priority > proc[roundRobin].priority || (proc[i].priority == proc[roundRobin].priority && time[i] < time[roundRobin])) {
            roundRobin = i;
      }
    }
  } 
